    ...
    in
    {
      ...
      config = {
        ...
        xdg.configFile."niri/config.kdl".text = ''
          ...
          // Bindings follow the old hand-written config at
          // github.com/mikl/dotfiles wherever the two disagreed, so the muscle
          // memory from it still applies. Where that config drove a tool this
          // machine does not have — fuzzel, hyprlock, brightnessctl — the key
          // is kept and pointed at the Noctalia equivalent instead.
          binds {
              ...
              // Volume and brightness stay on Noctalia rather than the old
              // config's wpctl/brightnessctl calls: same keys, but this way the
              // OSD shows up, and brightnessctl is not installed here anyway.
              ...
              Ctrl+Mod+Comma  { consume-window-into-column; }
